"""Clone all 'bindings' category repos to a local folder."""
...
from config import get_all_repos, GITHUB_OWNER, get_github_token, BINDINGS_REPOS_DIR

console = Console()


def clone_bindings_repos(pull_existing: bool = False):
    """Clone all 'bindings' category repos to the bindings directory."""
    BINDINGS_REPOS_DIR.mkdir(parents=True, exist_ok=True)

    # Filter repos in the "bindings" category
    all_repos = get_all_repos()
    repos = [r for r in all_repos if r.category == "bindings"]
    token = get_github_token()

    console.print(f"[bold]Cloning {len(repos)} binding repos to {BINDINGS_REPOS_DIR}[/bold]\n")
    ...
    with Progress(
        ...
        for repo in repos:
            repo_dir = BINDINGS_REPOS_DIR / repo.name
    ...
    console.print(f"\n[bold]Repos at:[/bold] {BINDINGS_REPOS_DIR}")
