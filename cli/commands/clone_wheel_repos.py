"""Clone all wheel repos to a local folder."""
...
from config import get_repos_by_category, GITHUB_OWNER, get_github_token, WHEEL_REPOS_DIR

console = Console()


def clone_wheel_repos(pull_existing: bool = False):
    """Clone all wheel repos to the wheel_repos directory."""
    WHEEL_REPOS_DIR.mkdir(parents=True, exist_ok=True)

    repos = get_repos_by_category("wheels")
    token = get_github_token()

    console.print(f"[bold]Cloning {len(repos)} wheel repos to {WHEEL_REPOS_DIR}[/bold]\n")
    ...
    with Progress(
        ...
        for repo in repos:
            repo_dir = WHEEL_REPOS_DIR / repo.name
    ...
    console.print(f"\n[bold]Repos at:[/bold] {WHEEL_REPOS_DIR}")
